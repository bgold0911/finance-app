...
export default function HomePage() {
  return (
    <>
      {/* Hero */}
      <section className="bg-[#111F42] text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
          <p className="text-[#EAB73E] text-sm font-bold uppercase tracking-widest mb-4">
            Personal Finance Tools
          </p>
          <h1 className="text-4xl sm:text-5xl font-bold leading-tight mb-6">
            Your Financial Future,
            <br />
            <span className="text-[#EAB73E]">Simulated.</span>
          </h1>
          ...
          <Link
            href="/tools/monte-carlo"
            className="inline-block bg-[#EAB73E] text-[#111F42] font-bold text-base px-8 py-4 rounded-lg hover:bg-yellow-400 transition-colors"
          >
            Try the Monte Carlo Simulator →
          </Link>
        </div>
        <div className="h-1 bg-[#EAB73E]" />
      </section>

      {/* Stats bar */}
      <section className="bg-[#1F3568] text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 grid grid-cols-3 gap-4 text-center">
          <div>
            <p className="text-2xl font-bold text-[#EAB73E]">1,000</p>
            <p className="text-sm text-gray-300 mt-1">Simulations per run</p>
          </div>
          <div>
            <p className="text-2xl font-bold text-[#EAB73E]">97 years</p>
            <p className="text-sm text-gray-300 mt-1">Of historical return data</p>
          </div>
          <div>
            <p className="text-2xl font-bold text-[#EAB73E]">100% free</p>
            <p className="text-sm text-gray-300 mt-1">No account required</p>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <h2 className="text-2xl font-bold text-[#111F42] mb-2">All Tools</h2>
          ...
          {TOOLS.map((tool) => (
            <div
              ...
              className={`bg-white rounded-xl border border-gray-200 p-6 flex flex-col gap-4 shadow-sm transition-shadow ${
                tool.available ? "hover:shadow-md hover:border-[#111F42]" : "opacity-70"
              }`}
            >
              ...
              <div>
                <h3 className="font-bold text-[#111F42] text-lg mb-1">{tool.title}</h3>
                <p className="text-gray-500 text-sm leading-relaxed">{tool.description}</p>
              </div>
              {tool.available ? (
                <Link
                  href={tool.href}
                  className="mt-auto inline-block text-center bg-[#111F42] text-white font-semibold text-sm px-4 py-2.5 rounded-lg hover:bg-[#1F3568] transition-colors"
                >
                  Open Tool →
                </Link>
              ) : (
